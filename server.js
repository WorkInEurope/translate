...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (data) => {
    ...
    if (msg.type === 'create') {
      ...
      rooms[code] = { A: ws, B: null, pair: msg.pair || null };
      myRoom = code;
      mySide = 'A';
      ...
      myRoom = code;
      mySide = 'B';
      // Στείλε στον worker το ζεύγος γλωσσών που διάλεξε ο host κατά τη δημιουργία -
      // έτσι δεν χρειάζεται να το ξαναδιαλέξει (βλ. prompt-one-to-one.md)
      ws.send(JSON.stringify({ type: 'joined', code, pair: rooms[code].pair }));
